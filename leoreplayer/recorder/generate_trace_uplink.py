...
def main():
    for i in range(1, cnt+1):
        ...
        subprocess.Popen([f"iperf3 -u -b {saturation_rate} -c {des_ip} -t {iperf_duration} &"], shell=True)

        ping_output_path = f'{trace_host_path}/{folder_path}/{output_file_name}.txt'
        with open(ping_output_path, "w") as f:
            subprocess.Popen([f"ping {des_ip} -i 0.005 -D &"], shell=True, stdout=f, stderr=subprocess.STDOUT)
            time.sleep(iperf_duration)
            os.system("sudo pkill -f 'ping|tcpdump|iperf'")

        with create_ssh_connection(des_ip, ssh_user, ssh_key_path) as ssh:
            if ssh:
                ssh_command(ssh, "sudo pkill -f 'tcpdump|iperf'")
