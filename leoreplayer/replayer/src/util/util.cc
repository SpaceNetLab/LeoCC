...
uint16_t get_last_octet(const sockaddr& sa) {
    const sockaddr_in* sa_in = reinterpret_cast<const sockaddr_in*>(&sa);
    const char* ip_cstr = inet_ntoa(sa_in->sin_addr);

    std::string ip(ip_cstr);
    size_t pos = ip.rfind('.');
    if (pos != std::string::npos) {
        std::string last_octet_str = ip.substr(pos + 1);
        uint16_t last_octet = static_cast<uint16_t>(std::stoi(last_octet_str));
        return last_octet;
    }

    return 256;  // default, invalid
}