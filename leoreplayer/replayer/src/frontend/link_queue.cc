      ...
      finished_( false )
{
    ...
    if (link_name.find("Uplink") != std::string::npos) {
        size_t last_slash_pos = filename.find_last_of("/");
        string base_filename = filename.substr(last_slash_pos + 1);

        string queue_log_filename = "queue_log_" + base_filename;
        queue_log_.reset(new ofstream(queue_log_filename.c_str()));
        if ( not queue_log_->good() ) {
            throw runtime_error( "Queue log error opening for writing" );
        }

        *queue_log_ << "Enter time(ns); Leave Time(ns); Queueing delay(ns); Content Length" << endl;
    }
...
{
    const uint64_t now = raw_timestamp();
    ...
    while ( next_delivery_time() <= now ) {
        ...
        while ( bytes_left_in_this_delivery > 0 ) {
            ...
            if ( packet_in_transit_bytes_left_ == 0 ) {
                ...
                output_queue_.emplace( move( packet_in_transit_.contents ), move(packet_in_transit_.arrival_time) );
            }
        }
    }
}
...
void LinkQueue::write_packets( FileDescriptor & fd )
{
    while ( not output_queue_.empty() ) {
        fd.write( output_queue_.front().first );
        if (queue_log_) {
            uint64_t queueing_delay = raw_timestamp() - output_queue_.front().second;
            *queue_log_ << output_queue_.front().second << " " << raw_timestamp() << " " << queueing_delay << " " << output_queue_.front().first.size() << endl;
        }
        output_queue_.pop();
    }
}
